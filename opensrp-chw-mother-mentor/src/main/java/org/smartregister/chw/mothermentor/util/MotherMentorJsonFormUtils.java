...
import static org.smartregister.chw.mothermentor.util.Constants.ENCOUNTER_TYPE;
import static org.smartregister.chw.mothermentor.util.Constants.MOTHERMENTOR_VISIT_GROUP;
...
public class MotherMentorJsonFormUtils extends org.smartregister.util.JsonFormUtils {
    public static final String METADATA = "metadata";

    public static Triple<Boolean, JSONObject, JSONArray> validateParameters(String jsonString) {

        JSONObject jsonForm = toJSONObject(jsonString);
        JSONArray fields = mothermentorFormFields(jsonForm);
    ...
    public static JSONArray mothermentorFormFields(JSONObject jsonForm) {
        if (jsonForm == null) {
            return null;
        }
    ...
    public static Event processJsonForm(AllSharedPreferences allSharedPreferences, String
        ...
        if (Constants.EVENT_TYPE.MOTHER_MENTOR_SCREENING.equals(encounter_type)) {
            encounter_type = Constants.TABLES.MOTHERMENTOR_SCREENING;
        } else if (Constants.EVENT_TYPE.MOTHER_MENTOR_SERVICES.equals(encounter_type)) {
            encounter_type = Constants.TABLES.MOTHERMENTOR_SERVICES;
        } else if (Constants.EVENT_TYPE.MOTHERMENTOR_CONTACTS.equals(encounter_type)) {
            encounter_type = Constants.TABLES.MOTHERMENTOR_CONTACTS;
        }
    ...
    protected static FormTag formTag(AllSharedPreferences allSharedPreferences) {
        ...
        formTag.appVersion = MotherMentorLibrary.getInstance().getApplicationVersion();
        formTag.databaseVersion = MotherMentorLibrary.getInstance().getDatabaseVersion();
        return formTag;
    }
    ...
    public static void tagEvent(AllSharedPreferences allSharedPreferences, Event event) {
        ...
        event.setClientApplicationVersion(MotherMentorLibrary.getInstance().getApplicationVersion());
        event.setClientDatabaseVersion(MotherMentorLibrary.getInstance().getDatabaseVersion());
    }
    ...
    public static JSONObject getFormAsJson(String formName) throws Exception {
        return FormUtils.getInstance(MotherMentorLibrary.getInstance().context().applicationContext()).getFormJson(formName);
    }
        ...
        for (Map.Entry<String, String> map : jsonStrings.entrySet()) {
            ...
            while (local_fields.length() > x) {
                try {
                    ...
                    obj.put(MOTHERMENTOR_VISIT_GROUP, map.getKey());
                    fields_obj.add(obj);
                } catch (JSONException e) {
                    Timber.e(e);
                }
