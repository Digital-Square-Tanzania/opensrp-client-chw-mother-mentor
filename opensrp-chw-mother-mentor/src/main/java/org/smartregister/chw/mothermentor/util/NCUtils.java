...
public class NCUtils {
    ...
    public static org.smartregister.Context context() {
        return MotherMentorLibrary.getInstance().context();
    }
    ...
    public static SimpleDateFormat getSourceDateFormat() {
        return new SimpleDateFormat(MotherMentorLibrary.getInstance().getSourceDateFormat(), Locale.getDefault());
    }

    public static SimpleDateFormat getSaveDateFormat() {
        return new SimpleDateFormat(MotherMentorLibrary.getInstance().getSaveDateFormat(), Locale.getDefault());
    }
    ...
    public static void processHomeVisit(EventClient baseEvent, SQLiteDatabase database, String parentEventType) {
        try {
            Visit visit = MotherMentorLibrary.getInstance().visitRepository().getVisitByFormSubmissionID(baseEvent.getEvent().getFormSubmissionId());
            if (visit == null) {
                ...
                if (StringUtils.isNotBlank(parentEventType) && !parentEventType.equalsIgnoreCase(visit.getVisitType())) {
                    String parentVisitID = MotherMentorLibrary.getInstance().visitRepository().getParentVisitEventID(visit.getBaseEntityId(), parentEventType, visit.getDate());
                    visit.setParentVisitID(parentVisitID);
                }

                if (database != null) {
                    MotherMentorLibrary.getInstance().visitRepository().addVisit(visit, database);
                } else {
                    MotherMentorLibrary.getInstance().visitRepository().addVisit(visit);
                }
                    ...
                    for (Map.Entry<String, List<VisitDetail>> entry : visit.getVisitDetails().entrySet()) {
                        if (entry.getValue() != null) {
                            for (VisitDetail detail : entry.getValue()) {
                                if (database != null) {
                                    MotherMentorLibrary.getInstance().visitDetailsRepository().addVisitDetails(detail, database);
                                } else {
                                    MotherMentorLibrary.getInstance().visitDetailsRepository().addVisitDetails(detail);
                                }
                            }
                        }
                    }
